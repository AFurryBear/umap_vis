...
%% LT 4/22/15 - IMPORTANT NOTE - trig status here is determined using the pitch threshold in config file. therefore not necessarily same as real trigger.

%% LT 4/22/15 - added saving of trig status info to the AllTrigs Data field
%% LT 11/25/14 -
% Modified from EvTAFv4 to:
% 1) provide outputs not just for triggers that passed freq threshold (as in original code), but to also output triggers that did not pass threshold (but still match the template).
% This makes it analogous to get_trigt2_v4_LT, which I used rpevious for evtafv4 analysis.  the problem with that code is that it does not handle repeat counters.
% 2) Will always put out both actual triggers (i.e. real was WN delivered?), and also All triggers to the ND structure.  Added the latter as a subfield, and did not change anything about previous stuff.
...
for iND=1:length(ND) % how many templates
    for iCnt=1:length(ND(iND).CntRng) % how many columns in template?
        if (ND(iND).CntRng(iCnt).Mode==1) % birdtaf No.
        ...
        % Initiate fields for AllTrigs saving (added by LT)
        ND(iND).AllTrigs.TriggerTimes=[];
        ND(iND).AllTrigs.FFVals=[];
        ND(iND).AllTrigs.AmpVals=[];
        ND(iND).AllTrigs.RepeatNumber=[];
        ND(iND).AllTrigs.TrigStatus=[];

    end
end


% NOW GO THROUGH ALL THE DATA FRAMES (time bins in this song file).
...
for dataind=PREBUFIND:NFFT:(length(dat)-NFFT+1)
    ...
    for iND=1:length(ND)
        ...
        % critereon is met yet? (i.e. trigger a set time after detect)
        %
        if (ND(iND).Detect==1)
            if (ND(iND).TimeSinceDet >= ND(iND).DelayToContingen)
                ...
                if (FreqMode==0)
                    ...
                    if (FundFreq<=FreqTHMax)
                        TRIGGER = TRIGGER & 1; % NOTE: TRIGGER already has values from Amp clust.
                    else
                        ND(iND).Detect=0;
                        TRIGGER=0;
                    end
                ...
                % This time bin was potentially triggered (whether triggered or not depends ONLY on
                % freq and ampl).  Save information for this bin. (LT)
                ND(iND).AllTrigs.readme = 'LT (12/2/14) - added this. It records data for all triggers regardless of whether ampl and freq crossed criteria.  Makes this analogous to get_trig2_v2_LT';
                ND(iND).AllTrigs.TriggerTimes=[ND(iND).AllTrigs.TriggerTimes; CurrentTime];
                ND(iND).AllTrigs.FFVals=[ND(iND).AllTrigs.FFVals;FundFreq];
                ND(iND).AllTrigs.AmpVals=[ND(iND).AllTrigs.AmpVals;Vol];
                ND(iND).AllTrigs.RepeatNumber=[ND(iND).AllTrigs.RepeatNumber;ND(iND).RepeatCount];
                ND(iND).AllTrigs.TrigStatus=[ND(iND).AllTrigs.TrigStatus; TRIGGER];
