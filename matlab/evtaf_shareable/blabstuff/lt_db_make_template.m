function lt_db_make_template(batchname)
...
% end
make_temp.parameters.batch_name = batchname;
dd = cd;
ix = strfind(dd,filesep);
birdname = dd(ix(end-2)+1:ix(end-1)-1);

% [birdname, ~, ~]=lt_get_birdname_date_from_dir(1);
make_temp.parameters.birdname = birdname;%input('birdname: ','s');
% make_temp.parameters.batch_name = 'batch.catch.keep';
...
if(1)    
    ...
    firstcol=make_temp.parameters.slices{i}(1);
    lastcol=make_temp.parameters.slices{i}(end);
    TemplName=[make_temp.parameters.birdname make_temp.parameters.prent '_' make_temp.parameters.syllables(i) '_' make_temp.parameters.postnt '_' num2str(firstcol) 'to' num2str(lastcol)  '.dat'];
    wrt_templ(TemplName,make_temp.template{i});
end
