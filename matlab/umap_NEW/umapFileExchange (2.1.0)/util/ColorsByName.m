...
%
    ...
    properties
        file;
        props;%key: name original case canonincal; value: java String 0 to 255 unrounded
    ...
    methods
        function this=ColorsByName(file)
            if nargin<1
                file=ColorsByName.DefaultFile;
            end
            this.file=file;
            this.map=BasicMap;
        ...
        function setColors(this, file)
            ...
            changed=0;
            it=java.util.HashSet(this.props.keySet).iterator;
            while it.hasNext
                key=it.next;
                name=strtrim(key);
                try
                    ...
                    if this.map.containsKey(nameLower)
                        warning('Case insensitivity: "%s"=[%s] overriden by [%s]\n', ...
                            key, String.Num2Str(str2num(char(strColor)), ' '), ...
                            String.Num2Str(floor(this.map.get(nameLower)*255), ' '));
                        this.props.put(java.lang.String(nameLower), strColor);
                        changed=changed+1;
                        continue;
        ...
        function chosenFile=compareBackup(this, pick)
            if nargin>1 && pick
                chosenFile=File.Ask([this.file '.*'], 'Pick colors backup');
    ...
    methods(Static)
        function file=DefaultFile(propFile)
            ...
            homeAg=fullfile(File.Home, '.autoGate', ColorsByName.FILE);
            if exist(file, 'file')
